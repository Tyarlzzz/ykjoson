...
try {
    ...
    $barongGownStmt = $conn->prepare("
        ...
        AND pc.product_name IN ('Barong', 'Gowns')
    ");
    ...
    foreach ($barongGownItems as $item) {
        ...
        } elseif ($item['product_name'] === 'Gowns') {
            $gowns_qty = intval($item['quantity']);
            $gowns_total = $gowns_qty * $gowns_price;
        }
    }

    // Step 7: Calculate grand total
    $grand_total = $clothes_total + $comforter_total + $barong_total + $gowns_total;
    ...
    $updateClothes = $conn->prepare("
        ...
        AND product_code_id IN (
            ...
            AND product_name NOT IN ('Barong', 'Gowns')
        )
    ");
    ...
    // Step 11: Update BARONG items (per piece pricing)
    if ($barong_qty > 0) {
    ...
    // Step 12: Update GOWN items (per piece pricing)
    if ($gowns_qty > 0) {
        $updateGown = $conn->prepare("
            ...
            AND product_code_id IN (
                SELECT code_id FROM `product codes`
                WHERE business_type = 'Laundry System'
                AND product_name = 'Gowns'
            )
        ");
        $updateGown->execute([
            ':weight_kg' => 0.00,
            ':price_per_piece' => $gowns_price,
            ':total' => $gowns_total,
            ':order_id' => $order_id
        ]);
    }
