...
<main class="font-[Switzer] flex-1 p-8 bg-gray-50 overflow-auto">
    <div class="w-full px-1">
        ...
        <div class="flex mb-0">
            ...
            class="px-10 py-1 bg-blue-600 text-white font-semibold border-l-2 border-gray-400 rounded-t-2xl -ml-3 z-0">Riders</a>
            <a href="archived.php"
                ...
                <div class="bg-white rounded-2xl shadow-xl p-6 sticky top-8">
                    <div class="mb-6">
                        <div class="grid grid-cols-2 gap-4 mb-4">
                            ...
                            <div>
                                <label
                                    class="block text-xs font-bold text-gray-700 mb-3 uppercase tracking-wide">Number</label>
                            ...
                            <tbody>
                                ...
                                foreach ($riders as $rider): ?>
                                    <tr class="hover:bg-gray-100">
                                        <td class="px-4 py-2 border-b border-gray-300"><?= htmlspecialchars($rider['id']) ?>
                                        </td>
                                        <td class="px-4 py-2 border-b border-gray-300">
                                            <?= htmlspecialchars($rider['name']) ?></td>
                                        <td class="px-4 py-2 border-b border-gray-300">
                                            <?= htmlspecialchars($rider['phone']) ?></td>
                                        <td class="px-4 py-2 border-b border-gray-300 truncate overflow-hidden text-ellipsis whitespace-nowrap max-w-[200px]">
                                            <?= htmlspecialchars($rider['address']) ?></td>
                                        <td class="px-4 py-2 border-b border-gray-300 text-center">
                                            <a href="rideredit.php?id=<?= $rider['id'] ?>&name=<?= urlencode($rider['name']) ?>&phone=<?= urlencode($rider['phone']) ?>&address=<?= urlencode($rider['address']) ?>">
                                                <!-- icons dito -->
                                                <button type="button"
                                            ...
                                            <a href="deleterider.php?id=<?= $rider['id'] ?>" onclick="return confirm('Are you sure you want to delete this rider?');">
                                                <!-- icons dito -->
                                                <button type="button"
                                                    class="bg-red-500 text-white px-3 py-1 rounded-lg font-semibold transition">
                                                    Delete
                                                </button>
                                            </a>
                                        </td>
