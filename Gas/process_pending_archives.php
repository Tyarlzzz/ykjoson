...
foreach ($pendingArchives as $archive) {
    if ($currentTime >= $archive['archive_at']) {
        ...
        try {
            ...
            $stmt = $conn->prepare("SELECT status FROM orders WHERE order_id = ? AND business_type = 'Gas System'");
            $stmt->execute([$orderId]);
            $order = $stmt->fetch(PDO::FETCH_ASSOC);
            
            if ($order && $order['status'] === 'Paid' && !GasArchivedOrder::isOrderArchived($orderId)) {
                if (GasArchivedOrder::archiveOrder($orderId)) {
